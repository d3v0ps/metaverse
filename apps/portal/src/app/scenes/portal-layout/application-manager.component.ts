...
import { userApplications } from "@central-factory/applications/data/base/user-applications.data";
...
@Component({
  ...
  template: `
    <ng-container *ngFor="let group of applicationGroups$ | async, let i = index">
      <cf-window [width]="group.width" [height]="group.height"
        [x]="group.x" [y]="group.y" [zIndex]="group.zIndex"
        [visible]="true" [executePostDisplayActions]="true"
        [backdrop]="false" [maximizable]="true"
        [ngStyle]="{
        ...
        <ng-container class="window-header__content">
          <ng-container *ngIf="group.applications[0].application?.icons as icons">
            <cf-svg-icon *ngIf="icons[0]" [src]="icons[0].src"></cf-svg-icon>
          </ng-container>
          <ng-container *ngIf="group.applications.length === 1 && group.applications[0]">
        ...
        <ng-container class="window-body__content">
          <ng-container *ngIf="group.applications.length === 1">
            <cf-application-view [application]="group.applications[0].application"
               [localResolver]="localResolver"></cf-application-view>
          </ng-container>
          <ng-container *ngIf="group.applications.length > 1">
            <cf-tabset>
              <cf-tab *ngFor="let event of group.applications" [title]="event.application.name"
                [icon]="event.application.icons && event.application.icons[0]?.src || ''">
                 <cf-application-view [application]="event.application"
                    [localResolver]="localResolver"></cf-application-view>
              </cf-tab>
            </cf-tabset>
          </ng-container>
  ...
  `,
  providers: [
    PortalLocalApplicationsResolver
  ],
  styles: [
    `
...
export class ApplicationManagerComponent implements OnInit {
  ...
  applicationGroups$ = this.applicationDisplayState.groupedApplications$.pipe(
    map(groupedApplications => {
      return Object.entries(groupedApplications).map(([groupId, applications], index) => ({
  ...
  ngOnInit() {
    if (!this._document.defaultView?.innerHeight || !this._document.defaultView?.innerWidth) {
      setTimeout(() => {
        this.ngOnInit();
      }, 100);
      return;
    ...
    this.applicationDisplayState.open({
      ...
      application: userApplications[2],
      settings: {
        show: true,
        // width: '1000px',
        // height: '1200px',
    ...
    this.applicationDisplayState.open({
      name: ApplicationOpenCommand.name,
      application: userApplications[1],
      settings: {
        groupId: '1',
        show: true,
        ...layout.rightPanel
    ...
    this.applicationDisplayState.open({
      name: ApplicationOpenCommand.name,
      application: userApplications[4],
      settings: {
        groupId: '1',
        show: true,
        ...layout.rightPanel
