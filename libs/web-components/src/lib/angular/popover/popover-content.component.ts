...
@Component({
  selector: 'cf-popover-content',
  template: `
    <div
      #popoverDiv
      class="popover-{{ effectivePlacement }} popover-content popover {{
        parentClass
      }}"
      [ngClass]="{
        sm: size === 'small',
...
export class PopoverContentComponent implements AfterViewInit, OnDestroy {
  ...
  protected positionElements(
    ...
    this.effectivePlacement = pos0 = this.getEffectivePlacement(
      pos0,
      hostEl,
      targetEl
    );
  ...
  protected getEffectivePlacement(
    placement: string,
    ...
    const overflows = {
      ...
      positionTopRight: {
        top: hostElBoundingRect.top - targetElement.offsetHeight < 0,
        right:
          hostElBoundingRect.right + targetElement.offsetWidth >
          this.windowWidth,
      },
      positionRight: {
        top:
    ...
    if (desiredPlacement === PopoverPlacement.Right) {
      ...
      if (overflows.positionRight.right && overflows.positionRight.top) {
        if (
          overflows.positionBottomRight.right &&
          overflows.positionBottom.right
        ) {
          return PopoverPlacement.BottomLeft;
        ...
        overflows.positionRight.right &&
        overflows.positionRight.bottom
      ) {
    ...
    if (desiredPlacement === PopoverPlacement.BottomRight) {
      ...
      if (
        overflows.positionBottomRight.bottom &&
        overflows.positionBottomRight.right
      ) {
        if (overflows.positionTop.top) {
          return PopoverPlacement.Left;
    ...
    if (desiredPlacement === PopoverPlacement.Bottom) {
      ...
      if (overflows.positionBottom.bottom && overflows.positionBottom.left) {
        ...
        overflows.positionBottom.bottom &&
        overflows.positionBottom.right
      ) {
        return PopoverPlacement.TopLeft;
    ...
    if (desiredPlacement === PopoverPlacement.BottomLeft) {
      ...
      if (
        overflows.positionBottomLeft.bottom &&
        overflows.positionBottomLeft.left
      ) {
        if (overflows.positionTop.top) {
          return PopoverPlacement.Right;
        } else if (overflows.positionTop.left) {
