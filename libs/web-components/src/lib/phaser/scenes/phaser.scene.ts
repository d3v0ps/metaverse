...
export class PhaserScene extends Scene implements BaseScene {
  ...
  constructor(
    ...
    this.layers = sceneLayers
      .map((layer) =>
        isSprite(layer) ? { id: layer.id, url: layer.properties.src } : null
      )
      .filter((layer) => (layer ? true : false)) as {
      id?: string;
      url: string;
    }[];
  }
  ...
  preload() {
    ...
    (this.load as any).AwaitFile(
      (successCallback: () => void, failureCallback: () => void) => {
        ...
        forkJoin(
          this.sceneLayers
            .filter((layer) => layer.properties.src)
            .map((layer) =>
          ...
          .pipe(
            map(
              (layers) =>
                layers.filter((layer) => layer !== null) as GameLayer[]
            ),
            tap((layers) => {
              this.spriteLayers = layers.filter((layer) => isSprite(layer));
              this.animationLayers = this.sceneLayers.filter(
                (layer) => isAnimation(layer) || isAnimationGroup(layer)
              );
            }),
  ...
  create() {
    ...
    this.spriteLayers.forEach((layer) =>
      isSprite(layer) ? this.playLayerAnimation(layer, this.animationId) : null
    );
  }

  private loadSpritesheets(layers: GameLayer[]) {
    return forkJoin(
      layers.map((spriteLayer, i) =>
        this.resourceExists(spriteLayer.properties.src).pipe(
          ...
          tap((layer: any) => {
            try {
              ...
              this.logger.log(
                'Phaser/SpriteSheet/Loaded',
                { name: layer.properties.name, src: layer.properties.src },
                LogLevel.Info
              );
            } catch (err) {
              this.logger.log(
                'Phaser/SpriteSheet/LoadedError',
                {
  ...
  private addSprites(layers: GameLayer[]) {
    return layers.forEach((layer, i) => {
      ...
      const exists = this.sprites.find(
        (sprite) => sprite.texture.key === layerId
      );
      ...
      try {
        ...
        this.logger.log('Phaser/Sprite/Added', {
          layerId,
          name: layer.properties.name,
        ...
        this.logger.log(
          'Phaser/Sprite/AddedError',
          { layerId, name: layer.properties.name, error: err },
          LogLevel.Error
        );
      }
    });
  ...
  private resourceExists(resource: string) {
    return this.http.get(resource, { responseType: 'text' }).pipe(
    ...
    layers.forEach((layer, i) => {
      if (isAnimationGroup(layer)) {
        layer.properties.animations.forEach((animation, i) => {
          ...
          this.logger.log('Phaser/AnimationGroup/Created', {
            group: layer.name,
  ...
  private createAnimation(
    { id, name, properties }: GameLayer<Animation>,
    parent?: GameLayer<AnimationGroup>
  ) {
    ...
    const textureLayer = this.sceneLayers.find(
      (layer) => layer?.name === properties.textureKey
    );
    ...
    const animationKey = `${textureLayer.id}/${
      parent?.properties.name ? parent.properties.name + '/' : ''
    }${properties.name}`;
    ...
    this.logger.log('Phaser/Animation/Created', {
      key: animationKey,
      start,
      end,
      row,
  ...
  private playLayerAnimation(layer: GameLayer, animation: string) {
    ...
    const sprite = this.sprites.find(
      (sprite) => sprite.texture.key === layer.id
    );
    ...
    this.logger.log('Phaser/Animation/Started', {
      layer: layer.id,
      key: animation,
    });
  }
