...
export const generateTypeScriptGqlTypesFromFile = async (
  input: string,
  output: string
): Promise<void> => {
  ...
  logger.verbose(
    `[${input.replace(
      process.cwd(),
      ''
    )}]: Generating gql types for the following root interfaces: [${schema.definitions[
      'Root'
    ].anyOf
...
export const generateTypescriptGqlTypesFromFolder = async (
  inputFolder: string,
  outputFolder = resolve(inputFolder, '__generated__', 'gql')
): Promise<void[]> => {
  ...
  return Promise.all(
    files.map((file) =>
      generateTypeScriptGqlTypesFromFile(
        resolve(inputFolder, file),
        resolve(outputFolder, file.replace('.d.ts', '.gql.ts'))
      )
    )
  );
};
