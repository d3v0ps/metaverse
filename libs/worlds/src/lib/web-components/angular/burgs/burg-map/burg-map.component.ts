...
export class BurgMapComponent {
  ...
  @Input() set burg(burg: Burg | undefined) {
    ...
    const sizeRaw =
      2.13 *
      Math.pow(
        ((burg.population || 0) * this.world.map.settings.populationRate) / 7,
        0.385
      );
    ...
    const hub = this.world.map.cells.cells[burg.cell || 0].road > 50;
    const walls = burg.walls || 0;
    const plaza = burg.plaza || 0;
    const temple = burg.temple || 0;
    const shantytown = burg.shanty || 0;
    const biome = this.world.map.cells.cells[burg.cell || 0].biome;
